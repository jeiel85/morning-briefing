  ...
  if (!briefing || briefing.items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <h1 className="mb-2 text-2xl font-bold">Today&apos;s Briefing</h1>
        <p className="mb-8 text-neutral-500">Your morning digest is ready to brew.</p>
      ...
      <div className="mb-8 flex items-end justify-between border-b border-neutral-200 pb-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">
            {briefing.generatedAt.toLocaleDateString()} · {briefing.items.length} items · {briefing.mode.replace("_", " ")}
          </p>
        </div>
      </div>
        ...
        {briefing.items.map((item) => (
          <article key={item.id} className="rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
            <div className="mb-3 flex items-center gap-3 text-xs">
              ...
              <span className="ml-auto text-neutral-400">{item.finalScore.toFixed(2)}</span>
            </div>
            ...
            {item.validationStatus === "limited" && (
              <p className="mb-4 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
                This item was summarized without AI. Source text is shown as-is.
              </p>
            )}
            ...
            <div className="flex flex-wrap gap-1.5 border-t border-neutral-100 pt-3">
              {(["useful", "not_useful", "duplicate", "block_source"] as const).map((type) => (
                <form key={type} action={saveFeedback.bind(null, item.id, type)}>
                  <button
                    type="submit"
                    className="rounded-md border border-neutral-200 px-2.5 py-1 text-xs text-neutral-500 transition-colors hover:border-neutral-300 hover:bg-neutral-50 hover:text-neutral-700"
                  >
                    {type === "useful" ? "👍" : type === "not_useful" ? "👎" : type.replace(/_/g, " ")}
                  </button>
                </form>
              ))}
            </div>
